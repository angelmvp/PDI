    ...
    public Image convertirRGBtoHIS(int selector) {
        ...
        for (int y = 0; y < alto; y++) {
            for (int x = 0; x < ancho; x++) {
                ...
                I = (rojo + verde + azul) / (3.0 * 255);  // Normalizando la intensidad
                ...
                H = H / 360.0 * 255.0;  // Normalizando el tono
                S = S * 255;            // Normalizando la saturación
                I = I * 255;            // Mantener la intensidad normalizada
        ...
        Image imagenNueva = padre.createImage(new MemoryImageSource(ancho, alto,
                imageBuffered.convertirInt2DAInt1D(nuevaImagen, ancho, alto),
                0, ancho));
        return imagenNueva;
    }
