...
public class Aritmeticas {
    ...
    public Aritmeticas (Image imagen1,Image imagen2){
    ...
    public Image MultiplicacionDeImagenes(){
        int [][] imagenInt = new int[altoMax][anchoMax];
        for(int y=0; y<altoMax; y++) {
            for (int x=0; x<anchoMax;x++){
                int min;
                ...
                if(y>=alto1 || x>=ancho1){
                    ...
                    max=Math.max(matrizImagen1[y][x], matrizImagen2[y][x]);
                    nuevoGris = (multiplicacion-min)*(255/max-min);
                }
                nuevoGris=this.validar(nuevoGris);
                color=new Color(nuevoGris,nuevoGris,nuevoGris);
                imagenInt[y][x] = color.getRGB();
            }
        }
    ...
    public Image DivisionDeImagenes(){
        int [][] imagenInt = new int[altoMax][anchoMax];
        for(int y=0; y<altoMax; y++) {
            for (int x=0; x<anchoMax;x++){
                int min;
                ...
                if(y>=alto1 || x>=ancho1){
                    ...
                    nuevoGris= (int) (matrizImagen1[y][x]/b);
